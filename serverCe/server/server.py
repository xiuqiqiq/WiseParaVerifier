...
def serv(conn, addr):
    ...
    size = 1024
    len_to_recv = None
    while len_to_recv is None or len(data) < len_to_recv:
        try:
            d = conn.recv(size).decode()
    ...
    if cmd[0] == COMPUTE_REACHABLE:
        ...
        ord_str = cmd[-1]
        gen_smv_process(name, content.encode(), ord_str)
        ...
        if cmd[2] in smv_pool:
            ...
            # print("res:",res)
            if res:
                conn.sendall((SPLIT_CHAR.join([OK, res.decode()]) if res.decode() else WAITING).encode())
            else:
        ...
        if cmd[2] in smv_pool:
            ...
            conn.sendall(SPLIT_CHAR.join([OK, res.decode()]).encode())
            # conn.sendall(SPLIT_CHAR.join([OK, res]).encode())
        else:
            conn.sendall(ERROR.encode())
    elif cmd[0] == SMV_QUIT:
        """
        ...
        # There are many SPLIT_CHARin smv file, so should concat the parts splited
        name = cmd[2]
        content = SPLIT_CHAR.join(cmd[3:]).encode()
